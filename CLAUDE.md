...
## 현재 단계
v3 본 구현 진행 중. 최신 릴리스 **v0.0.51** (2026-04-27) — v0.0.49 NTP 정밀 재도입 측정에서 **seek 연타 시 호스트·게스트 5~118초 어긋난 채 재생** 문제 발견(csv frame diff 분석). v0.0.50 cooldown 시도는 더 악화시켜 revert. v0.0.51 진짜 fix: 호스트 `_broadcastObs`가 `_engine.latest`(100ms poll 캐시) 대신 매번 `await _engine.getTimestamp()`로 fresh fetch. 캐시가 syncSeek 직후 stale → 게스트가 옛 host_vf 기반 anchor establish → 게스트는 옛 위치로 seek된 채 유지되어 호스트와 절대 어긋남 누적이 원인. (HISTORY (45) 분석 + revert 정황 + fix 상세). 미검증 — 다음 측정에서 seek 연타 시 frame diff 100ms 이내로 정상화 확인 필요. 미적용: (c) outputLatency 비대칭 자동 보정, (d) Tab A7 Lite xrun 회피, BT 워밍업 옵션 A, 게스트 anchor sanity check, 호스트 syncSeek 직렬화 — 측정 후 효과 부족하면 추가.
