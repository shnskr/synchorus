...
## 현재 단계
v3 본 구현 진행 중. 최신 릴리스 **v0.0.55** (2026-04-27) — v0.0.54 측정에서 frame diff abs_mean **261ms** 회귀 발견 (사용자 "안 맞음" 매칭). vf-sanity 0회 발동 — 임계 500ms 안 넘는데도 fd 250~948ms 일관 어긋남. v0.0.55에서 (1) vf-sanity 외삽 시간 가드 (obsAge > 1000ms 시 skip), (2) vf-sanity 임계 500→200ms로 낮춤, (3) clearTempFiles에 logger.stop 추가 + cleanupSync async — csv flush 보장. 잠재 위험: 임계 낮춤으로 anchor reset 너무 자주 발생 시 무한 loop 가능 (다음 fix 후보: outputLatency 자동 보정 EMA). 미적용: (c) outputLatency 비대칭 자동 보정, 호스트 측 syncSeek 직렬화, 메시지 처리 큐 직렬화, (d) Tab A7 Lite xrun 회피, BT 워밍업 옵션 A.
