...
## 현재 단계
v3 본 구현 진행 중. 최신 릴리스 **v0.0.50** (2026-04-27) — v0.0.49 측정 후 발견된 idx 86 outlier(drift 계산 race) fix. `_handleSchedulePlay`에서 anchor 즉시 무효화 + `_latestObs=null` + 1초 cooldown으로 await yield window 동안 stale obs 기반 잘못 정렬 차단. 다음 측정에서 outlier 0개 확인 필요.

이전: **v0.0.49** (2026-04-27) — NTP 예약 재생 정밀 재도입 (B 단계: NTP only, prewarm 제외). v0.0.47 1차 시도가 race + sequence ordering 부재로 drift 60초 회귀 → v0.0.48 롤백 후 한 세션에서 (a) sequence number 도입 + (b) race 완전 제거 + (e) 호스트도 schedule 적용 (대칭) 한꺼번에 적용. 핵심 변경: 호스트 syncPlay/syncSeek(재생 중)/syncPause 모두 wall+200ms NTP 약속 + `schedule-play{seq, startWallMs, fromVf}`/`schedule-pause{seq}` broadcast. 게스트 `_lastSeenSchedSeq` 가드로 stale 메시지 무시 + 호스트 권위 보장(`_handleAudioObs` 자동 start/stop 분기 완전 제거). 합류 게스트만 `_scheduleFromObs` catch-up. dead code (`_startGuestPlayback`/`_stopGuestPlayback`) 제거. **측정·검증 미진행 — 다음 세션 1순위는 drift csv 측정 + (42) edge case 재현 여부 확인 + 회귀 시 v0.0.48로 즉시 롤백 (롤백 기준: drift abs 평균 4ms 초과 or max 60초 초과)**. 미적용: (c) outputLatency 비대칭 자동 보정, (d) Tab A7 Lite xrun 회피, BT 워밍업 옵션 A. 진단 로그 `[SYNCPLAY-HOST]`/`[SYNCPAUSE-HOST]`/`[GUEST] schedule-play seq=...`/`[GUEST] catch-up via _scheduleFromObs` 유지.

**다음 작업 (v0.0.50+ 또는 v0.0.49 측정 후 분기)**:
- **측정 우선** (v0.0.49 합격 시): (c) outputLatency 비대칭 자동 보정 → 처음 40초 BT 잔여 단축 시도
- **회귀 시**: drift abs 평균 4ms 초과 시 v0.0.48 호출 흐름으로 부분 롤백, 변경 항목 bisect (a/b/e 중 어느 게 원인?)
- **별도 트랙**: (d) Tab A7 Lite oboe `requestPause` xrun 회피 — `cancelSchedule`이 stream을 죽이지 않으므로 NTP에선 영향 줄어들 것으로 기대, 측정 후 결정
