...
## 현재 단계
v3 본 구현 진행 중. 최신 릴리스 **v0.0.54** (2026-04-27) — v0.0.53 측정에서 정지/재생/seek 연타 후 마지막 10초 frame diff 1.3초 누적 발견. 분석 결과 idx 135-137 host_vf 1.6초 정지(호스트 syncPause 후) + 게스트 계속 진행 → schedule-pause 처리 race로 게스트가 정지 신호 못 받음. v0.0.49에서 stale 위험으로 제거한 `obs.playing=false` 자동 stop 분기를 streak 2회(~1초) 가드와 함께 다시 추가 — 메시지 race 안전망. v0.0.51 fresh `_broadcastObs`로 stale 위험은 작아진 환경. 미적용: 호스트 측 syncSeek 직렬화(B), 메시지 처리 큐 직렬화, (c) outputLatency 자동 보정, (d) Tab A7 Lite xrun 회피, BT 워밍업 옵션 A.
