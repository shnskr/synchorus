...
0. **게스트 3명 입장 불가 fix (HIGH, 사용성 직접 영향)** — 갤럭시 3대 같은 방 연결 시 호스트+1명만 유지되고 무한 핑퐁 발견 (2026-04-29). 원인: `home_screen.dart:136`의 `name='Guest'` 하드코딩 + `p2p_service.dart:232` v0.0.32 stale 정리가 같은 이름 peer 강제 destroy. fix 옵션: A) `device_info_plus`로 디바이스명 기반 고유 이름 + B) stale 비교를 이름 AND IP로 강화. 권장: A+B 동시. 상세: HISTORY (51).
