...
## 현재 단계
v3 본 구현 진행 중. **현재 main = v0.0.54 (v0.0.53 + 게스트 이름 충돌 핑퐁 fix)** — 2026-05-01 세션에 (51)에서 식별된 다중 게스트 입장 불가 버그를 A+B 동시 fix (`device_info_plus`로 디바이스명 발급 + stale 비교를 name AND ip로 강화). 실측 검증은 같은 모델 2대 이상 환경 필요. 그 전 v0.0.53는 2026-04-28 세션에 v0.0.51~v0.0.55 시도 → v0.0.50 reset → syncSeek debounce 시도 → v0.0.52 진단 컬럼 → v0.0.53 anchor 중복 호출 fix. 상세: `docs/HISTORY.md` (45)~(52).
...
0. ~~게스트 3명 입장 불가 fix~~ — **v0.0.54 (52)에서 A+B 동시 fix 완료** (`device_info_plus` + name AND ip 강화). 실측 검증은 다음 세션 (같은 모델 2대 이상 환경 필요). 상세: HISTORY (52).
