...
## 현재 단계
v3 본 구현 진행 중. 최신 릴리스 **v0.0.52** (2026-04-27) — v0.0.51 fresh `_broadcastObs` 측정에서 frame diff 146초 어긋남 자동 회복 2초 걸림 + 사용자 청감 "조금 틀어짐". `drift_ms`(framePos 변화율 기반)와 `frame diff`(virtualFrame 직접 차이)가 본질적으로 다른 단위라 drift_ms는 호스트 콘텐츠 점프 자체를 못 보는 구조적 한계 발견. v0.0.52 fix: `_recomputeDrift`에 virtualFrame 기반 `vfDiffMs` 별도 계산 + sanity check (vfDiff>500ms AND drift<50ms 시 anchor 강제 reset → fresh obs 기반 재정렬). 무한 loop 방지 300ms cooldown + `event=vf-sanity` csv 기록. 다음 측정에서 frame diff 1초 이내 회복 확인 필요. 미적용: 호스트 측 syncSeek 직렬화 (B), (c) outputLatency 자동 보정, (d) Tab A7 Lite xrun 회피, BT 워밍업 옵션 A.
