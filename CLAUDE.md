...
## 현재 단계
v3 본 구현 진행 중. 최신 릴리스 **v0.0.56** (2026-04-27) — v0.0.55 측정에서 fd abs_mean 358ms로 더 나빠짐 (예고된 무한 loop 발생). 코드 재정독에서 `_tryEstablishAnchor` 중복 호출 버그 발견 (seekToFrame + accum 두 번). v0.0.56 fix: (1) vf-sanity 임계 200→500 복원, (2) outputLatency cap 200ms 추가 (anchor + drift + fallback 3곳), (3) anchor 중복 호출 버그 fix. cap 200ms는 양쪽 내장 systemic 부정확 보고(+290ms) 영향 제한 + BT 정상 비대칭(200ms 안)은 영향 없음. 진짜 fix는 (c) outputLatency 자동 보정 EMA — 측정 후 잔여 시 추가. 미적용: (c), 호스트 측 syncSeek 직렬화, 메시지 처리 큐 직렬화, (d) Tab A7 Lite xrun 회피, BT 워밍업 옵션 A.
...
- **v0.0.54 (2026-04-27 (48))**: v0.0.53 측정에서 정지/재생/seek 연타 후 마지막 10초 frame diff 1.3초 누적. csv 분석으로 호스트 정지 1.6초 동안 게스트만 계속 진행 (schedule-pause 처리 race로 게스트 정지 신호 못 받음 추정). **v0.0.54 fix**: `_handleAudioObs`에 `obs.playing=false` 안전망 다시 추가. streak 2회(~1초) 연속이면 강제 정지. v0.0.49에서 stale 위험으로 제거한 분기지만 v0.0.51 fresh broadcast로 위험 작아진 환경. **미검증**: 다음 측정에서 마지막 frame diff 100ms 이내 + streak debug log 발동 빈도. 미적용: 호스트 직렬화(B), 메시지 처리 큐 직렬화. 상세: `docs/HISTORY.md` 2026-04-27 (48).
- **v0.0.55 (2026-04-27 (49))**: v0.0.54 측정에서 fd abs_mean 261ms 회귀 발견 (vf-sanity 0회 발동). 분석: 외삽 거리 1초+에서 vfDiff 거짓말 + 임계 500ms 너무 관대. **v0.0.55 fix 3가지**: (1) vf-sanity 외삽 시간 가드 (obsAge>1000ms skip + reset), (2) sanity 임계 500→200ms, (3) clearTempFiles에 logger.stop 추가 + cleanupSync async (csv flush 보장). **잠재 위험**: 임계 낮춤 무한 loop 가능. **미검증**: 다음 측정에서 fd abs_mean 50ms 이내 + sanity 발동 빈도. 미적용: (c) outputLatency 자동 보정 EMA — 다음 후보. 상세: `docs/HISTORY.md` 2026-04-27 (49).
- **v0.0.56 (2026-04-27 (50))**: v0.0.55 측정에서 예고된 무한 loop 발생 (fd abs_mean 358ms, fallback 4배). 코드 재정독에서 `_tryEstablishAnchor` 중복 호출 버그 발견 — `_engine.seekToFrame` + `_seekCorrectionAccum` 두 줄이 두 번 호출되어 게스트 같은 위치 두 번 seek + accum 2배 누적. v0.0.49부터 있던 잠재 버그. **v0.0.56 fix**: (1) vf-sanity 임계 200→500 복원, (2) outputLatency cap 200ms (anchor + drift + fallback 3곳), (3) 중복 호출 fix (한 번만). cap 200ms는 systemic 부정확(+290ms) 영향 제한 + BT 정상 비대칭(200ms 안) 영향 없음. **미검증**: 다음 측정에서 fd abs_mean v0.0.54 baseline(261ms) 또는 더 작게. 진짜 fix (c) outputLatency 자동 보정 EMA는 측정 결과 보고 추가. 상세: `docs/HISTORY.md` 2026-04-27 (50).
