...
class _HostPageState extends State<HostPage> {
  ...
  Future<void> _refreshIps() async {
    ...
    String best = '—';
    final wifi = candidates.where((c) =>
        c.iface.startsWith('wlan') || c.iface.startsWith('en'));
    if (wifi.isNotEmpty) {
      best = wifi.first.ip;
  ...
  /// 네이티브 getTimestamp에서 실제 하드웨어 sampleRate를 받아 갱신.
...
class _GuestPageState extends State<GuestPage> {
  ...
  double? _iosIoBufferMs;
  String _iosLatencyDetail = '';
  ...
  Future<void> _guestPollOnce() async {
    try {
      ...
      if (sr != null) mSampleRate = (sr as num).toInt();
      // iOS 진단 데이터
      final olMs = result['outputLatencyMs'];
      if (olMs != null) _iosOutputLatencyMs = (olMs as num).toDouble();
      final tlMs = result['totalLatencyMs'];
      if (tlMs != null) _iosTotalLatencyMs = (tlMs as num).toDouble();
      final ioBuf = result['ioBufferDurationMs'];
      if (ioBuf != null) _iosIoBufferMs = (ioBuf as num).toDouble();
      final nodeL = (result['nodeLatencyMs'] as num?)?.toDouble() ?? 0;
      _iosLatencyDetail = 'hw=${_iosOutputLatencyMs?.toStringAsFixed(1)} + io=${_iosIoBufferMs?.toStringAsFixed(1)} + node=${nodeL.toStringAsFixed(1)}';
      _guestPolls++;
      if (ok) {
        _guestOkCount++;
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _kvRow(t, 'sampleRate', '$mSampleRate'),
                    _kvRow(t, 'totalLatency',
                        _iosTotalLatencyMs != null ? '${_iosTotalLatencyMs!.toStringAsFixed(1)}ms' : '—'),
                    _kvRow(t, 'latency분해', _iosLatencyDetail),
                    _kvRow(t, 'ioBuffer',
                        _iosIoBufferMs != null ? '${_iosIoBufferMs!.toStringAsFixed(1)}ms' : '—'),
                    if (_csvDebug.isNotEmpty)
                      _kvRow(t, 'csv', _csvDebug),
                  ],
                ),
              ),
            ),
