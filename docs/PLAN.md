...
- ⏳ Algorithm latency를 `outputLatencyMs`에 반영 (sync 자동 보정) — ⚠️ **정정(2026-06-10 (143))**: **정적 항은 이미 반영됨** — Android v0.0.112(SoundTouch `INITIAL_LATENCY`+batch)/iOS v0.0.122(`timePitch.latency`), drift median 0.24·acoustic +7.84로 검증. **남은 건 out-ring 동적 점유만**(`kSTOutRingFrames=8192`≈0~170ms 변동, `oboe_engine.cpp:627`서 anchor 출렁임 우려로 1단계 의도적 제외). 종합 P2 — 동적 반영은 효용<비용 후보(측정 수치 이미 양호). close 검토.
- ✅ **30분 stress + 측정 — underrun 카운터(v0.0.124, (143)) + 첫 실측((144)) 완료**. `oboe_engine.cpp` decode(`:843`/`:873`)/ST(`:862`) atomic 카운터 → 호스트 csv `host_*_under_*` 8컬럼 + 게스트 `[UNDERRUN][guest]` logcat. 측정음원 `assets/measure_scale_30min.mp3`(도레미 100ms+900ms, 8초주기, 30분). **실측 결론((144))**: underrun은 **재생 시작 워밍업 1회성**(ST 154ms + decode 13ms, 배속 무관, 정상재생 0). 깨끗한 1배속 sync 양호(Android vfDiff 7ms / iPhone 17ms, fallback 3%, verify_fail 0). ⏳ **남은**: out-ring 동적 점유 반영(효용<비용 후보) / iPhone 속도·음정 과도기 오차(§I-6).
