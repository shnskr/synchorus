...
- ⏳ **T2 (다음 트랙, 강제주입 테스트 후)**: vf-정지 watchdog → `reopenStream()`+`_resetDriftState()`. 게이트(`_playing`+seek쿨다운). **reopen은 HAL framePos 리셋→P2P drift 폭발**이라 reopen 후 `_resetDriftState`+obs reopen-epoch 필수(검토 🔴). reopenStream: stop→close/ST out-ring clear/백오프. Android 게이트(iOS=`rebuildEngineAndResume` 보유, fast-follow). on/off 컴파일 상수 + 디버그 강제-stuck 훅으로 테스트.
- ⏳ **T3 (폴리시)**: Oboe `onErrorAfterClose` 자동 재오픈. ④ fallback Exclusive→Shared.
...
- ✅ **내부 테스트 v0.0.132 Play 배포본 실유저 검증 (2026-06-18~19)** — git push 완료. AAB(vc3, `--no-tree-shake-icons`) Play 내부테스트 업로드 → **Play 앱 서명 재서명본**에서 미니플레이어/세로/배너 서빙/스와이프 종료 전부 정상 확인. 무료 체험 재검증은 다른 구글 계정 필요(현 테스트 계정은 환불-후-권한회수 불가로 영구 프로 — Google: 환불 시 "권한 회수" 미체크면 사후 회수 불가).
- 🔄 **프로덕션(정식 출시) 준비 중 (2026-06-19, 최신 2026-06-30)** — **현 단계: 비공개 테스트(closed) 7일째 — 테스터 12명 모집 완료, 14일 카운트 진행 중(D+7, ~2026-07-07 충족 예정).** Play Console 작업 상태:
  - ✅ **앱 콘텐츠 전부 완료**: 개인정보처리방침(Google Sites 호스팅+URL), 콘텐츠 등급(전체이용가, 디지털구매=예), 데이터 안전성(AdMob 4유형 신고: 대략위치/앱활동/진단/기기ID, 전송중 암호화=예, 삭제요청=아니요), 로그인 세부정보(로그인 없음), 광고=있음, 금융/건강/정부앱=해당없음, **포그라운드 서비스 권한**(미디어재생 + 백그라운드 재생 시연 YouTube 영상 링크)
  - ✅ **스토어 등록정보**: 간단/자세한 설명(솔로 연습 각도 포함, 바탕화면 `Synchorus_스토어설명.txt`), 앱아이콘 512(app.png 리사이즈), 피처그래픽 1024×500(PIL 자작, `~/Desktop/store_graphics/`). ✅ **폰 스크린샷 완료(2026-06-19)**
  - ✅ **게시 차단 해제 = 한국 전자상거래법 (완료, 2026-06-30)**: IAP 판매 계정이라 계정 세부정보(개발자 계정 → 계정 세부정보) **3칸 입력 필수** — `사업자 등록 번호` + `전자상거래 라이선스 번호`(=통신판매 신고번호) + `전자상거래 라이선스 대행사`(=신고기관 시/군/구청). ✅ **사업자등록증 발급 완료(2026-06-19)**. ✅ **통신판매업 신고 정부24 접수(2026-06-19)** — 앱 개발자는 **구매안전서비스 비적용**(앱마켓 판매라 에스크로 확인증 불요, "정보통신망 전송 재화" 체크+소명), 판매방식=기타 / 취급품목="디지털 콘텐츠(스마트폰 앱)". 등록면허세(~40,500원) 납부. **✅ 통신판매업 신고 완료(2026-06-30) → 신고번호로 계정 세부정보 3칸 입력 → 게시 차단 해제됨.** (겸업=근로계약서+취업규칙 확인 전제. 대안 IAP 제거 무료출시는 보류.)
  - ✅ **확정(2026-06-19, 게시 개요 실측)**: 전자상거래법 계정 세부정보 3칸이 **비공개(closed) 테스트 "변경사항 제출"까지 차단**한다. 게시 개요에서 "검토를 위해 변경사항 12개 제출" 비활성 + 빨간 "문제 1개 = 계정 세부정보 업데이트(대한민국 법규)" 배너 실측. **내부 테스트는 통과했으나 closed 제출은 막힘** → 앞서 '게시 차단이 closed까지 막는지 확인 필요'였던 가설 해소. 즉 **통신판매 신고번호 = closed 게시→14일 카운트의 critical-path 시작점**(내부 테스트로 미리 못 당김).
  - 🔴 **프로덕션 액세스 요건**: 신규 개인계정(2023.11.13 이후)은 **비공개 테스트(closed) 테스터 12명 × 연속 14일** 채워야 프로덕션 신청 가능. ⚠️ 내부 테스트는 미인정 — **closed 트랙**이라야 함. ⚠️ 테스터 "이메일 등록" ≠ 카운트 시작 — 실제 **opt-in(웹 링크 수락+설치)** 12명이 14일 유지돼야 함. 게시 전 제출하면 테스터는 "항목을 찾을 수 없습니다"(릴리스 미게시 상태). (Alpha 자동생성 + 사용자가 추가 트랙 1개 더 만듦 → 릴리스·테스터·국가 **한 트랙에 통일** 필요)
  - ℹ️ **패키지 이름 `com.synchorus.synchorus`**(android/app/build.gradle.kts:35, ios bundle id 동일) — 게시 후 변경 불가 + 설치 이력 있어 재사용도 불가, **사용자에게 비노출**(보이는 건 앱 이름 "Synchorus"). 그대로 유지 결정(2026-06-19).
- 🔄 **남은 순서**: ① ✅사업자등록 / ✅통신판매 신고 완료(2026-06-30) → ✅계정 세부정보 3칸 입력 → ✅게시 차단 해제 ② ✅폰 스크린샷 / ✅트랙 국가(대한민국) ③ ✅**비공개 테스트(closed) 게시** ④ 🔄**테스터 12명 모집 완료 → 연속 14일 카운트 진행 중(2026-06-30 기준 7일째, ~2026-07-07 충족 예정)**(현 단계, critical path) ⑤ ⏳프로덕션 액세스 신청 ⑥ ⏳버전 0.0.132→1.0.0 프로덕션 승격
- ⏳ **iOS 라운드**: AdMob iOS 실 ID + UMP, 스플래시/아이콘 실기기 검증, 알림 권한 iOS 경로, release 회귀.
- ⏳ **🆕 환불/자격철회 시 프로 자동 해제 — 출시 후 과제 (2026-06-22 조사)**. 증상: 비공개 테스트 중 프로 결제 → Play에서 환불 + "자격 삭제(revoke)"까지 했는데 앱 프로모드 안 풀림. **원인 2겹**: ① 앱 코드에 프로를 *내리는* 경로가 아예 없음 — `purchase_service.dart:72-87` `_onPurchaseUpdate`는 `purchased/restored`면 `setPro(true)`만, 환불/취소는 무처리. `isPro_v1`(`app_providers.dart:16,32-36`)이 한번 true면 SharedPreferences에 영구 잔존(`_load()`가 그대로 복구). 주석(`app_providers.dart:12-13`)은 "진실의 출처=스토어 계정"이라지만 **하향 reconcile 미구현**. ② `in_app_purchase`(3.3.0)는 환불 비인지 — revoke 후에도 `restorePurchases()`가 `restored`로 돌려주는 알려진 버그(flutter [#107755](https://github.com/flutter/flutter/issues/107755)/[#124518](https://github.com/flutter/flutter/issues/124518)/[#158373](https://github.com/flutter/flutter/issues/158373)). 클라 단독 환불감지 불가. **제대로 된 해법 = 서버 검증**: Google Play Developer API `purchases.products.get`(`purchaseState` PURCHASED/CANCELLED) + Voided Purchases API + RTDN / Apple App Store Server API(`revocationDate`/`revocationReason`) + Refund Notif V2. 단 이 API들은 **서비스 계정 키(재무권한) 필요 → 백엔드에서만 호출**(키를 앱에 심으면 디컴파일로 전체 결제내역 노출). **사이징**: Firebase Cloud Function 1개(요청 시만 깨어남, idle=0)로 충분 — purchaseToken 받아 라이브 1회 조회만(DB 불필요), 256MB×~1s ≈ 무료한도(월 200만 호출/40만 GB-초) 내 월 ~150만 검증 무료. ⚠️ 외부 네트워크 호출이라 **Blaze 플랜(카드 등록) 필요하나 실비 $0**. **결정(2026-06-22)**: 디버그 리셋 버튼 **안 넣기로 함**. closed 테스트 중 막힌 기기는 **앱 데이터 삭제/재설치**로 `isPro_v1` 리셋(단 revoke 전파 ~5분 + ① 때문에 재실행 restore가 재점화 가능). 환불 자동 강등은 이 Cloud Function(B) 방식으로 **출시 후** 구현. 1회 결제 환불 누수(소수)는 그때까지 감수.
...
- ✅ **Algorithm latency를 `outputLatencyMs`에 반영 — 효용<비용 close (2026-06-11 (146))**. 정적 항(SoundTouch `INITIAL_LATENCY`+batch)은 이미 반영됨(Android v0.0.112 / iOS v0.0.122 `timePitch.latency`), drift median 0.24·acoustic +7.84로 검증. 남은 out-ring 동적 점유(`kSTOutRingFrames=8192`≈0~170ms 변동, `oboe_engine.cpp:627`)는 **반영 안 함으로 결정** — 측정 이미 양호(효용 불확실) + 동적값 가산 시 anchor 출렁임 우려(비용 확실). **재개조건**: BT 등 큰 비대칭 경로 체감 시 / 다른 큰 이슈 해결 후 마지막 병목 시 (그땐 raw 아닌 EMA smoothing). 상세 HISTORY (146).
- ✅ **30분 stress + 측정 — underrun 카운터(v0.0.124, (143)) + 첫 실측((144)) 완료**. `oboe_engine.cpp` decode(`:843`/`:873`)/ST(`:862`) atomic 카운터 → 호스트 csv `host_*_under_*` 8컬럼 + 게스트 `[UNDERRUN][guest]` logcat. 측정음원 `assets/measure_scale_30min.mp3`(도레미 100ms+900ms, 8초주기, 30분). **실측 결론((144))**: underrun은 **재생 시작 워밍업 1회성**(ST 154ms + decode 13ms, 배속 무관, 정상재생 0). 깨끗한 1배속 sync 양호(Android vfDiff 7ms / iPhone 17ms, fallback 3%, verify_fail 0). ⏳ **남은**: ~~out-ring 동적 점유 반영~~ → **(146) close** / iPhone 속도·음정 과도기 오차(§I-6).
- ✅ **iOS 실기기 첫 검증 (2026-06-10, v0.0.121, HISTORY (140))** — S947N 호스트 + iPhone 12 Pro 게스트(profile 빌드). transpose/speed 전파·seek·정지·resume(0:00 안 튐)·monotonic 안정 모두 ✅. **신규 크래시 fix**: "player did not see an IO cycle"(SIGABRT, speed+seek 연타 재현) → `engine.isRunning` 가드 + `objcTryCatch`(ObjC NSException catch) + `rebuildEngineAndResume` 3층 → 크래시 0 검증. ⏳ **잔여 트랙**:
  1. ✅ **iOS sync 오프셋 — v0.0.122 (141) 해결**. timePitch.latency(baseline 85.33ms, pitch 무관·speed 반비례) outputLatency 가산(Android v0.0.112 SoundTouch latency 대칭) → acoustic **emit_dt +7.84ms**(결함 B ~11ms 범위)로 fix 성공 확정(가설 A: 85ms = 실제 음향 지연). ⚠️ (140) "−13~−27ms(csv vfDiff) = timePitch 미반영" 진단은 **인과 오진** — vfDiff는 outLat fix에 순환 무감각(seek와 계산에 같은 outLatDelta 상쇄). **방법론 교훈: outLat 자체를 바꾸는 fix는 vfDiff(순환)/청감(미세차)으로 검증 불가 → acoustic 필수.**
  2. ⏳ **잔음 / seek 폭주 — v0.0.123 seek-notify coalesce(150ms)로 완화 ((142))**. 실측 정정: 주범은 framePos 붕괴/establish-REJECT(verify_fail 16)가 아니라 **host_seek 331 → 게스트 seekToFrame 331 = iOS 디코더 재생성**. baseline+일반 seek OK, **극한 막 조작엔 잔존**(디바운스 150 < seek 간격 median 176ms). 추가 = 디바운스 튜닝 or iOS 엔진 디코더 재사용(깊음). framePos 도메인 재설계는 잔재 재현 시.
  3. ⏸️ **② notification 핸들러 — 조건부 보류 close (2026-06-10 (141))**. interruption/routeChange/configChange 핸들러 0개가 크래시 근본 트리거였으나 v0.0.121 예외잡기(`objcTryCatch`)로 크래시 0 + (140) 결론 "notification 다 구현해도 race로 잔존 → 예외잡기가 유일 확실 차단" → 크래시 관점 추가가치 미미. 실익(예외 빈도↓/전화·이어폰 후 재개 UX) 미관찰(효용 불확실) + 라이프사이클 회귀 검증 비용 확실 → 효용<비용. **재개조건**: 크래시 자연 재발 or "전화 후 게스트 재생 미복귀" 등 실사용 불편 관찰 시 configChange/interruption 핸들러 추가(`rebuildEngineAndResume` 재사용).
  - 부수: CLI hung 정체 = 첫 연결 symbol 복사(622s→캐시 후 15s, CLAUDE.md 정정). idevicesyslog 진단 인프라 확보. Swift print는 syslog 미표시(stdout) → native 진단은 NSLog 필요.
...
- ✅ **v0.0.111 거짓말 패턴(vfDiff) re-anchor + speed 정규화 (HISTORY (123))** — 맥북 마이크 acoustic 측정으로 vfDiff = 실제 스피커 시차임을 확정(465ms 일치). vfDiff 중앙값 >150ms 시 anchor 리셋 + speed 정규화(vfDiff/speedFactor). vfDiff max 474→156ms, 2배속 224→23ms. tempo 디바운스(250ms)·계측(msgSeq)·tcpNoDelay 동반. ⏳ **미해결 (별도 트랙 — 한 번에 안 건드림, 다음 세션 하나씩)**:
  1. ✅ **isOffsetStable jitter → anchor 공백 — v0.0.120 fix (HISTORY (135))**. stable 판정을 "winMinRaw 2ms 비교"(RTT 노이즈 ±RTT/2에 깨짐) → **"RTT≤20 샘플이 5초 내 들어왔는가"**(`_lastGoodSampleMs`)로 재설계. 역할 분리(stable=anchor 타이밍, offset=EMA filtered). 검증(집 WiFi): stable true 82%(이전 fallback 109회→토글 4회), 청감 잘 맞음. ✅ **reject ~73% → close (HISTORY (139))**: reject는 RTT>30 품질 게이트(`sync_service.dart:314` reject 시 window/EMA/stable 변화 0 = 무해), 높은 건 WiFi doze/간섭 환경 탓이지 버그 아님. offset 안정(stdev 0.62)·청감 OK로 영향 없음((138)). 완화 다 역효과(임계↑=noisy로 offset 품질↓ / ping 주기↓=트래픽↑ / median 동적 임계=효용<비용). 재개조건: WiFi 매우 나빠 good 샘플 장기 공백 → fallback 의존도↑로 청감 틀어질 때. ✅ **anchor 주기 재발행 재측정 → 트랙 보류(close) (HISTORY (136))**: offset 안정화 후 realign 주기 발동(v0.0.121) 재측정 — realign 빈도↑ = vfDiff 악화(2초 -26 / 5초 -15 / baseline -5, 주기↑일수록 baseline 수렴), self-seek 음수 편향이 root(establish는 -5 정확). 청감 무영향(vfDiff -26도 "대체로 OK") → 효용<비용 → 1단계 롤백 + close. 재개 시 seek 아닌 rate-bend(SYNC_REDESIGN #5)로. 〈이하 과거 진단 기록〉 filtered offset 1.9ms 안정인데 raw RTT jitter(15~30ms)가 `_stableCount` 리셋 → anchor ~20초 안 박힘(그동안 fallback). ⚠️ **v0.0.112 타임아웃 강제 establish 시도 → 폐기 (HISTORY (124))**: 재입장 시엔 offset 자체가 없는(rawOff=0/rtt=0) 상태라 force가 틀린 위치에 박아 악화. "offset은 있고 판정만 막힘" 가정이 재입장엔 안 맞았음(단 (134) 후속 측정으로 재입장 8초는 측정 아티팩트로 해소 — 위 #5). **🆕 2026-06-05 anchor 주기 재발행(SYNC_REDESIGN) 1단계 측정으로 선행 확정**: offset 불안정(fallback 109회/2분)이 vfDiff를 거짓으로 만들어 anchor 트랙 측정을 오염 → **#1(offset 안정화)이 anchor 주기 재발행의 선행조건**. anchor 1단계 롤백, **#1 먼저** 후 anchor 재측정(사용자 합의 "하나씩 확실히"). 판정 임계 2ms vs 입력 노이즈 ±15ms 미스매치(`sync_service.dart:341-345`)가 root — winMinRaw 윈도우/임계 보강 방향.
  2. **150ms 임계 → 80~100ms 낮춤** (체감상 큼, staleness 마진 고려).
  3. **host HAL getTimestamp 간헐 실패** (framePos=-1, HISTORY (30) 재발).
  4. **게스트 engine 재시작 루프** (host seek 연타 + play/pause 토글 막 조작 트리거, "position 동기 표시인데 다른 부분 재생"). ⚠️ **"다른 부분 재생"은 ring overwrite((133), v0.0.118 fix)였을 가능성 높음** — seek/pause가 trigger라 prewarm/pause decode 폭주 조건과 일치. 재발 시 재확인.
  5. ✅ **재입장 시 clock sync ~8초 = 측정 아티팩트로 해소 (2026-06-05 (134) 후속 측정)**. v0.0.119로 재현: syncWithHost가 매번 즉시 정상 offset(completed=30, ~3초) 확보 — `rawOff=0/rtt=0`은 startPeriodicSync 전용 csv 컬럼(`lastRawOffsetMs/RttMs`)이 재입장 직후 곡 다운로드의 WiFi 점유로 주기 ping RTT 2648ms 폭주 → reject(>30ms)돼 8초간 안 채워진 것. fallback도 -12ms로 정확(offset 정상이라). (124) "재입장 청감 틀어짐"의 진짜 범인은 ring overwrite((133))/resume 0:00 점프((134))였음 — 둘 다 vfDiff 기반 측정이라 못 봤음. **부수 발견(실해 없음)**: `_syncRequestSeq` reset 누락 → 재입장 requestId 음수(rid 매칭만 되면 무해).
  6. ✅ **vfDiff 40~95ms 진동 — 소멸 확정 → close (2026-06-05 (138))**. (124) 재입장 후 관찰된 40↔90 왕복 진동은 v0.0.118(ring overwrite)/v0.0.119(resume 0:00) fix 후 재측정(재입장 5회, `measurements/reentry_2026-06-05.csv`)에서 **재현 안 됨** — 각 구간 폭 3~6ms. "(124) ring overwrite/resume 버그 오염" 가설 적중. 부수: fallback obs 의존성 분석으로 fallback-only 전환 리스크 확인(obs stale 시 잘못 seek = HISTORY (98)) → anchor main 합리적 확정. 결함 A 잔재(-7~-22)는 청감 OK + (136) close 유지. 상세 HISTORY (138).
- ✅ **v0.0.118 ring buffer overwrite fix — "시크바≠소리" (2026-06-05 (133))** — prewarm/pause 중 callback이 tail 동결인데 decodeLoop가 ring 가득 차도 wait_for timeout마다 계속 decode → head가 cap 초과 폭주 → ring modular(vf%cap) overwrite로 vf 위치에 미래 frame PCM 덮어씀 → 시크바 위치와 실제 소리 다른 구간(호스트 prewarm 22초 시 10초 어긋남). ring 도입(v0.0.76) 이래 잠복 — 비프/단일톤 회귀 + vf 기반 동기 측정(drift/vfDiff) 모두 무감지, **실제 음악 청감으로만 발견**. fix: wait_for 후 head-tail>=cap이면 decode skip. 검증: pause 38초 후 cap에서 멈춤(폭주 0).
- ✅ **v0.0.119 resume 0:00 점프 fix (2026-06-05 (134))** — 정지→재생 시 ts.ok=false(getTimestamp ErrorInvalidState) 구간에 poll이 _fallbackAlignment 호출 → monoMs=0(timeNs=-1)으로 외삽 폭주 → garbage seekTo(음수→0:00 clamp) → 게스트 0:00 튐. **v0.0.115 monotonic 전환 회귀**. fix: ts.ok=false 시 fallback skip(정렬만 skip, 재생은 콜백이 vf 진행). ⏳ **남은 트랙**: (a) 게스트 ~30-50ms 앞섬(기존 (125)/(126) anchor establish 오차, fallback drift -36/-31/-47) (b) 늦은 입장 pre-seek — engine.start 전 호스트 위치로 미리 seek해 0:00 잠깐 재생 제거(현재 ts.ok 회복 34-64ms라 체감 작음, BT 등 느린 경로 체감 시).
...
- ✅ **v0.0.114 realign + virtualFrame 시점 정합 — 톱니 근본 fix (HISTORY (128))** — (126) "±수십ms 변동(톱니)"의 진짜 원인 = `virtualFrame`(마지막 콜백 ~현재) vs `wallMs`(=framePos의 HAL timeNs 과거 시점) **시점 불일치** → 게스트 vfDiff 외삽이 HAL 지연 이중 카운트. `oboe_engine.cpp`에서 virtualFrame을 timeNs 시점으로 정렬 + realign(vfDiff 60ms 시 baseline fresh 재정렬, 150ms anchor=null 리셋 대체). 측정(transpose +5, 3분): drift vfDiff 30-60ms **148→0**, >60 **11→0**, min/max ∓108→∓27. **±50/±100 톱니 제거.** drift(framePos↔wall 정합)가 멀쩡했던 게 증거. iOS는 vf/framePos 둘 다 lastRenderTime이라 정합(보정 불필요). 진단: transpose 0/+5 둘 다 톱니(SoundTouch 무관) + obs_age 무관(외삽 거리 무관). ⚠️ **정정 → (129) 측정4로 진전**: 측정3 "톱니 제거"는 무효였으나(offset 점프로 vfDiff 거짓, raw guest_vf−host_vf +250~512ms = 청감 ~500ms 일치, seek_count 0), **(129) offset 정상 재측정**(filtered 192 변동 2.3, anchor drift 317 vs fallback 48)으로 ✅ **톱니fix 검증 성공**(vfDiff -19.5 일정, ±50 톱니 사라짐) + ✅ **"offset 고치면 500ms 잡힘" 실증**(offset 정상되니 500ms 사라지고 vfDiff -19.5). offset이 root 확정. ⏳ **다음 (확정 순서)**:
  1. **B (monotonic 전환) — offset 점프 면역 (진짜 1순위)** — ✅ **설계 완료 (2026-06-05 (130), [SYNC_REDESIGN.md](SYNC_REDESIGN.md) "(130)") → ✅ **구현+검증 완료 (v0.0.115, task #2~6, HISTORY (130))**. 검증(S947N+S901N, 2분): offset boot 1.1ms 변동(측정3 점프 제거), vfDiff **-19.5→-3.64**, seek_count 0, drift 0.59. ⚠️ **실측 함정**: AAudio `getTimestamp(BOOTTIME)`가 clockId 무시·MONOTONIC 반환 → 재생시간 음수(vf -89억) 버그 → iOS 대칭(MONOTONIC 받고 `outTimeNs`만 +sleep누적 BOOTTIME화)으로 수정.** scheduleStart는 dead path(v0.0.48) 확인 → 스킵. **데드코드 정리 트랙**: ✅ scheduleStart/cancelSchedule 경로 전체 삭제(v0.0.116, HISTORY (131)). ✅ `nowAsHostTime`+`localTimeToHost`/`hostTimeToLocal` dead 삭제(v0.0.117). prewarm/coolDown은 **유지**(coolDown이 iOS `unload()` 실사용 → dead 아님, HISTORY가 "dead"라 적었으나 오기). ⏳ **wall 병행 경로만 잔여** — iOS 실기기 검증 후 제거(검증 전 제거하면 회귀 시 대조 불가). 측정3 wall 점프 재발 방지. **Android `CLOCK_BOOTTIME` / iOS `mach_continuous_time`**(=`clock_gettime_nsec_np(CLOCK_MONOTONIC_RAW)`). ⚠️ **정정**: Darwin `CLOCK_MONOTONIC`은 deep sleep "포함"이나 NTP에 점프(REALTIME offset) → **금지**, raw 계열 필수(1차 소스 검증). iOS는 AVAudioTime.hostTime=mach_absolute_time 고정이라 native 변환 레이어 필요. Dart는 dart:ffi 직접 호출. 전수조사: 정렬 전부 wall 도메인(native가 monotonic을 일부러 wall로 역변환 `oboe_engine.cpp:686`). EMA min-RTT 로직은 점프 없어지면 그대로 둬도 됨.
  2. ✅ **결함 B-HAL 진단 완료 + 보정 보류(close) (2026-06-05 (132), HISTORY (132)/SYNC_REDESIGN (132))** — 맥북 마이크로 음향 시차 **~11ms 고정**(호스트 출력 먼저) 실측. vf_diff(−28→−6.8 출렁) 무관 + monotonic offset 안정 + drift(framePos)≈0 → **DAC 이후 출력단 하드웨어 비대칭**(SoundTouch 무관, transpose bypass 측정). csv out_lat(Oboe `calculateLatencyMillis`)가 DAC 이후 못 잡음(부호 반대 과소보고). 측정 인프라 `scripts/acoustic/`. 🚫 **보정 구현 보류** — 11ms 인지경계(효용↓) + 동적 캘리브레이션 비용↑·측정오차 악순환 + 더 큰 이슈(anchor 공백 ±240ms/재입장 8초) 우선. **재개조건**: BT 등 큰 비대칭 경로 체감 시 / 큰 이슈 해결 후 마지막 병목 시 (SYNC_REDESIGN (132) "결정").
  3. **vfDiff -19.5 position 편향** — seek 임계(20ms) 바로 아래라 방치(seek_count 0). 임계/보정 검토.
  - 측정 교훈: vfDiff는 offset 의존 외삽값 → offset 불안정 시 못 믿음. **anchor 박힘(drift vs fallback) + raw guest_vf−host_vf + acoustic 교차 검증 필수.**
- 📋 **전체 로드맵: [SYNC_REDESIGN.md](SYNC_REDESIGN.md)** — anchor 주기 재발행 / 미세 보정층 / 시계동기 강화 우선순위.
- ⏳ **speed 전환 과도기 (§I-6, 다음 트랙)** — speed/음정 조정 시 sync 틀어짐(사용자 보고). ⚠️ **(142) 정정: 네트워크 아님 확정** — RTT median 16ms인데 vfDiff 스파이크 ±100~224ms(RTT의 10배+). 원인 = **엔진/timePitch latency 과도기**(speed마다 timePitch.latency 85/64/128ms 점프 → 전환 시 outputLatency 비대칭 + 엔진 버퍼 재충전). 멈추면 −24ms 안정(전환 순간 8%만 튐). 기존 §I-6 "네트워크 지연" 가정 + "적용 시각 broadcast" 해법은 네트워크 보상 전제라 **재검토 필요**. acoustic 교차검증(vfDiff offset 의존) 권장. **설계 합의 선행**. 🆕 **(144) 실측 보강**: iPhone이 속도/음정 조정 시 Android보다 민감 — 2배속 측정서 iPhone vfDiff |절대|17→71ms(>100ms 25%), Android는 20ms로 덜 흔들림. **1배속(무조작)은 iPhone도 17ms로 양호** → 과도기 한정 이슈. iOS timePitch.latency가 speed별로 점프(85/64/128ms)하는 게 주범 후보. 일상엔 영향 없고 속도/음정 기능 쓸 때 iOS 과도기 오차로 참고.
- ✅ **시크바/시간 표시 정확도 — speed≠1.0서 정상 확인 → close (2026-06-10 (145))**. PLAN 후보는 v0.0.92(§I) 도입 때 남긴 미검증 항목(실버그 아님). 조사 결과 Duration=`totalFrames/sampleRate`(원본 길이)·Position=`virtualFrame/sampleRate`(vf가 콘텐츠 기준 진행)·Seek=절대 위치 모두 콘텐츠 timeline 기준으로 정상("2배속이면 표시도 2배 빠르게" = YouTube/Spotify 표준). Android 코드 확정 + iOS는 (144) 2배속 vfDiff 71ms 유지가 간접 실증(출력 기준이면 초당 0.5초 누적해 sync 붕괴했을 것) + iPhone 실기기 눈 확인. 상세 HISTORY (145).
- ⏸️ **iPhone 2배속 톤 누락 — realign 과잉 seek 가설 → 기록 후 보류 (2026-06-11 (148))**. 사용자 보고 "2배속 가끔 짧은 톤 누락". 추적: 단독 1대 재현 0(SoundTouch/82ms batch 무죄) → 어제 (144) csv 재분석서 `anchor_realign_vfdiff` **46회 전부 iPhone·2배속 구간 집중**(Android 0). realign = vfDiff중앙값>60ms 시 `seekToFrame`(`native_audio_sync_service.dart:1803`) → iOS 디코더 재생성 → 톤 건너뜀 가설. underrun 0인데 누락(seek는 무음 아닌 톤 건너뜀이라 카운터 무감지) **모순 해소**. **iOS 편중(발동=iPhone vfDiff, 아픔=iOS seek) + 후순위 → fix 보류**. 재개조건: iOS 트랙 재개. fix 후보: realign 임계 iOS 상향/신뢰도 게이트 · iOS seek 디코더 재사용((142) 잔음과 동일) · vfDiff 거짓말 acoustic 확정 후 realign 신뢰. **선행**: realign vfDiff 로깅 누락 fix(0→실값). 상세 HISTORY (148).
...
**결정 (2026-06-16, v0.0.127 구현): 배너 광고 + 일회성 "프로" IAP** — 구독 ❌ / 자체 서버 ❌. ⚠️ **2026-06-01 "광고 안 함" 결정을 뒤집음**(사용자 결정으로 하단 배너 추가). 근거·변경 이력 [DECISIONS.md](DECISIONS.md), 구현 [HISTORY.md](HISTORY.md) (155).
...
- **프로 적용 = 호스트 기준**: 호스트가 결제하면 그 방 게스트 무제한 + 배너는 각 기기의 프로 여부 기준.
- 가격은 5,000~9,900원 범위에서 출시 후 결정.

### 왜 이 구성인가 (구독❌, 자체서버❌, 배너는 추가됨)

- **구독 안 함**: 유틸+로컬 동작 앱은 "정적 기능에 월정액" 저항이 큼 (2026 능동 해지율 31%→47%, 유틸 앱 일회성 회귀 트렌드). 동기화는 "정해진 일을 잘하는 도구"라 일회성이 맞음.
- **배너 추가됨 (옛 "광고 안 함" 뒤집음)**: 옛 근거(재생 중 백그라운드라 노출↓·수익 미미)는 배너를 **플레이어 화면 하단**(조작 중엔 화면 봄)에 둬 일부 완화. 배너 단독 수익은 작고(eCPM 한국 낮음) **주 수익은 프로 IAP**, 배너는 보조. ATT/SKAdNetwork/개인정보처리방침 부담은 감수.
- **자체 서버 안 함**: `in_app_purchase` non-consumable + `restorePurchases`(서버 0, 클라 검증). 스토어 계정 기준 복원이라 재설치·기기변경 OK. RevenueCat은 구독·교차계정 식별 가치가 일회성+로그인없음 앱엔 미적용이라 채택 안 함.
...
~~무료로 먼저 출시(수익화 코드 0)~~ → **2026-06-16 (155) 수익화 코드 선구현 완료(v0.0.127)**. 출시 전 배너+IAP+2대 제한을 미리 넣음(전략 전환: 무료 먼저가 아니라 수익화 포함 출시). 남은 건 스토어 콘솔 등록(실 광고/상품 ID)뿐.
