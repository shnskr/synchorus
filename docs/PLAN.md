...
- ✅ **v0.0.114 realign + virtualFrame 시점 정합 — 톱니 근본 fix (HISTORY (128))** — (126) "±수십ms 변동(톱니)"의 진짜 원인 = `virtualFrame`(마지막 콜백 ~현재) vs `wallMs`(=framePos의 HAL timeNs 과거 시점) **시점 불일치** → 게스트 vfDiff 외삽이 HAL 지연 이중 카운트. `oboe_engine.cpp`에서 virtualFrame을 timeNs 시점으로 정렬 + realign(vfDiff 60ms 시 baseline fresh 재정렬, 150ms anchor=null 리셋 대체). 측정(transpose +5, 3분): drift vfDiff 30-60ms **148→0**, >60 **11→0**, min/max ∓108→∓27. **±50/±100 톱니 제거.** drift(framePos↔wall 정합)가 멀쩡했던 게 증거. iOS는 vf/framePos 둘 다 lastRenderTime이라 정합(보정 불필요). 진단: transpose 0/+5 둘 다 톱니(SoundTouch 무관) + obs_age 무관(외삽 거리 무관). ⚠️ **정정 (청감 + raw csv): "톱니 제거" 무효** — 측정3는 offset 불안정으로 vfDiff(±42)가 거짓이었고, raw guest_vf−host_vf = +250~512ms로 게스트 실제 ~500ms 어긋남(청감 일치, measure_audio 비프 정반대, seek_count=0 = 보정 0회). offset 부정확 시 외삽이 실제 어긋남을 지움 = 거짓말 패턴 재확인. 톱니fix·realign은 offset 안정에서만 검증 가능. ⏳ **잔여 (정정된 순서)**:
  1. **offset/clock sync 안정화 (진짜 1순위)** — anchor 안 박히는 근본(isOffsetStable raw RTT jitter #1 / 재입장 clock sync 지연 #5). offset 거짓이면 vfDiff 통째 거짓(측정3 실증) = 토대. 잡혀야 나머지 다 의미.
  2. **톱니fix·realign 재검증** — offset 안정 상태에서 ±50 톱니 실제 감소 재측정.
  3. **+16ms 편향** — offset 정상 재측정 후 판단 (측정3 +16은 offset 거짓 산물).
  - 측정 교훈: vfDiff는 offset 의존 외삽값 → offset 불안정 시 못 믿음. anchor 박힘 여부 + raw guest_vf−host_vf + acoustic 교차 검증.
