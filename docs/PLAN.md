...
- ⏳ **speed 전환 과도기 (§I-6, 다음 트랙)** — speed/음정 조정 시 sync 틀어짐(사용자 보고). ⚠️ **(142) 정정: 네트워크 아님 확정** — RTT median 16ms인데 vfDiff 스파이크 ±100~224ms(RTT의 10배+). 원인 = **엔진/timePitch latency 과도기**(speed마다 timePitch.latency 85/64/128ms 점프 → 전환 시 outputLatency 비대칭 + 엔진 버퍼 재충전). 멈추면 −24ms 안정(전환 순간 8%만 튐). 기존 §I-6 "네트워크 지연" 가정 + "적용 시각 broadcast" 해법은 네트워크 보상 전제라 **재검토 필요**. acoustic 교차검증(vfDiff offset 의존) 권장. **설계 합의 선행**.
