...
- 🔄 **프로덕션(정식 출시) 준비 중 (2026-06-19)** — Play Console 작업 상태:
  ...
  - 🟡 **게시 차단 = 한국 전자상거래법 (진행 중)**: IAP 판매 계정이라 계정 세부정보(개발자 계정 → 계정 세부정보) **3칸 입력 필수** — `사업자 등록 번호` + `전자상거래 라이선스 번호`(=통신판매 신고번호) + `전자상거래 라이선스 대행사`(=신고기관 시/군/구청). ✅ **사업자등록증 발급 완료(2026-06-19)**. ✅ **통신판매업 신고 정부24 접수 완료(2026-06-19)** — 앱 개발자는 **구매안전서비스 비적용**(앱마켓 판매라 에스크로 확인증 불요, "정보통신망 전송 재화" 체크+소명), 판매방식=기타 / 취급품목="디지털 콘텐츠(스마트폰 앱)". 등록면허세(~40,500원) 납부 후 **신고증 1~3일 대기 중**. → 신고번호 나오면 3칸 입력 → 차단 해제. (겸업=근로계약서+취업규칙 확인 전제. 대안 IAP 제거 무료출시는 보류.)
...
- ⏳ **남은 순서**: ① ✅사업자등록 / ✅통신판매 접수(신고증 1~3일 대기) → 신고증 나오면 **계정 세부정보 3칸 입력(게시 차단 해제)** ② ✅폰 스크린샷 / ✅트랙 국가(대한민국) ③ 비공개 테스트 게시(차단 해제 후) ④ 테스터 12명 opt-in → **14일 대기** ⑤ 프로덕션 액세스 신청 ⑥ 버전 0.0.132→1.0.0 프로덕션 승격
