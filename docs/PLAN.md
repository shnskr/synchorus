...
- 🔄 **프로덕션(정식 출시) 준비 중 (2026-06-19)** — Play Console 작업 상태:
  ...
  - ✅ **스토어 등록정보**: 간단/자세한 설명(솔로 연습 각도 포함, 바탕화면 `Synchorus_스토어설명.txt`), 앱아이콘 512(app.png 리사이즈), 피처그래픽 1024×500(PIL 자작, `~/Desktop/store_graphics/`). ⏳ **폰 스크린샷 2장+ 남음**
  - 🔴 **게시 차단 = 한국 전자상거래법**: IAP로 판매하는 계정이라 계정 세부정보에 **사업자등록번호 + 통신판매 신고번호 + 신고기관(시/군/구청)** 입력 필수. **→ 사용자가 사업자등록 + 통신판매업 신고 진행 결정**(겸업 = 근로계약서+취업규칙 둘 다 확인 전제. 등록 ~1주: 사업자등록 홈택스 무료 즉시~3일 / 통신판매업 정부24 등록면허세 ~4만원/년 + 구매안전서비스 확인증). **대안(IAP 제거 무료출시)은 보류.**
  - 🔴 **프로덕션 액세스 요건**: 신규 개인계정(2023.11.13 이후)은 **비공개 테스트(closed) 테스터 12명 × 연속 14일** 채워야 프로덕션 신청 가능. ⚠️ 내부 테스트는 미인정 — **closed 트랙**이라야 함. (Alpha 자동생성 + 사용자가 추가 트랙 1개 더 만듦 → 릴리스·테스터·국가 **한 트랙에 통일** 필요)
- ⏳ **남은 순서**: ① 사업자/통신판매 번호 → 계정 세부정보 입력(게시 차단 해제) ② 폰 스크린샷 + 트랙 국가(대한민국) ③ 비공개 테스트 게시 ④ 테스터 12명 opt-in → **14일 대기** ⑤ 프로덕션 액세스 신청 ⑥ 버전 0.0.132→1.0.0 프로덕션 승격
