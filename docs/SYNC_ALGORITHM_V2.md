...
7. ⏳ Crossfade(Option C) — 필요 시 (현재 transition click 매우 미세, 음악에선 안 들림). **해결법 (2026-06-11 (149) 정리, 보류 유지)**: click 원인 = pitch/tempo 변경 시 `mST.clear()` 생략(v0.0.102, `oboe_engine.cpp:155,166`)으로 mSTOutRing에 옛↔새 출력 이음새 불연속. 단일 SoundTouch 인스턴스라 진짜 crossfade 어려움. 옵션: **(A) 짧은 fade** — 변경 순간 수ms 페이드out→변경→in, click→미세 dip, 단일 인스턴스 가능, **권장** / (B) 이중 인스턴스 crossfade — 옛·새 병렬 N ms 겹침, 정확하나 CPU·메모리 잠깐 2배+위치/latency 정렬 복잡 / (C) 파라미터 ramp — transpose엔 글리산도(음 미끄러짐)라 부적합, speed엔 가능. ⚠️ **iOS는 `AVAudioUnitTimePitch`(OS 내장)라 내부 fade 불가** → 노드 출력단 별도 처리(Android와 구현 다름). 가성비: 미세 click이라 보류 합리적, 착수 시 A로 Android 먼저+iOS 별도. 상세 HISTORY (149).
...
### I-3 sync 알고리즘 영향 (핵심 차이) — **2026-06-01 (120) 측정으로 정정**

Transpose는 vf 진행 속도 무변경 → drift 계산 영향 0. 속도는 vf 진행이 speed배 → 외삽 영향. 당초 "같은 식 OK (자동 상쇄)"는 **framePos 경로 한정으로만 맞음**, vf 경로는 틀림(2배속 실측):

- **driftMs (framePos 외삽)**: `framePos`는 HAL DAC 카운터라 speed 무관 wall rate → 호스트/게스트 동일 진행, rate 비교 **자동 상쇄. speed 곱 불필요(견고)**. → 실제 seek 폐루프는 speed≠1.0이어도 안정.
- **vf 외삽 (anchor `hostContentFrame` / fallback / `vfDiffMs`)**: ❌ **자동 상쇄 아님**. vf는 호스트가 speed배 진행(`oboe_engine.cpp:834,851`)하는데 게스트 외삽은 1배속 가정 → obs staleness(최대 500ms)만큼 위치 과소추정. **실측**: 순수 2배속 vfDiff 음수 ~107ms(1배속은 <20ms). → **v0.0.104에서 vf 외삽에 `speedFactor=obs.speedX1000/1000` 곱**(anchor seek 위치 정확화). framePos 외삽은 그대로.
- **잔존**: 외삽 fix 후에도 2배속 vfDiff 잔차(raw guest-host 605ms를 외삽으로 상쇄하나 2배속 증폭). 단 driftMs 견고 → 청감 OK. **vfDiff가 2배속에서 과대보고**로 추정. 근본 완화는 obs broadcast 주기 단축.
...
### I-4 P2P 동기화 — **v0.0.103 보강**

- 호스트 setPlaybackSpeed → `audio-tempo {speedX1000}` broadcast + obs에 `speedX1000`/`transposeCents` 동봉.
- 게스트 `_onMessage` → `engine.setPlaybackSpeedX1000(...)` + **speed면 `_resetDriftState`**(P1-a, vf rate 변경에 따른 anchor stale 방지). transpose는 vf rate 무변경이라 제외.
- audio-url에 `playbackSpeedX1000`/`transposeCents` 동봉 → 늦게 들어온 게스트도 호스트 현재 값 적용.
- **P0 fix (v0.0.103)**: native `loadFile`이 cents/speed를 0/1000 강제 reset(안전망)하므로, 게스트는 **loadFile 후 재적용** 필수. 다운로드 전 적용(:911/916)은 reset에 덮임(v0.0.93 회귀). → HISTORY (120).
- **자가치유 (P1-b, v0.0.103)**: 단발 `audio-tempo`/`audio-pitch` broadcast 유실 대비. 게스트가 매 obs(500ms) `obs.speedX1000`/`transposeCents`와 자기 값 비교 → 불일치 시 재적용(speed면 anchor 리셋).

### I-5 검증 결과 (2026-06-01 (120)) + 미검증

**검증 완료 (실기기 S26+ 호스트 + S22 게스트)**:
- ✅ P0 (늦게 합류 speed 상실) — 게스트가 호스트 1.5x/2.0x 따라감.
- ✅ 외삽 fix 안정성 — 2배속에서 anchor_set 16→1, fallback 163→34, drift ~2ms 견고.
- ✅ 1배속 baseline / speed 고정 정상 재생 — 청감 OK.

**미검증 / 후속**:
- P1-b 자가치유 (WiFi 교란 시나리오 직접 재현)
- iOS 실기기
- 2배속 장시간 stress + underrun 카운터(현재 객관 카운터 없음, `oboe_engine.cpp:840/862`)
- Edge case (A-B, 메모리, transpose+속도 조합), 시크바/시간 표시(speed≠1.0)
- 2배속 vfDiff staleness 잔차 — obs broadcast 주기 단축 검토 (우선순위 낮음, 청감 OK)

### I-6 전환 스케줄링 — **신규 설계 (다음 트랙, 2026-06-01 (120) 발견)**

**문제 (실측)**: speed 전환 순간(특히 **2→1 감속**) 게스트가 일시적으로 호스트보다 앞서 재생(vfDiff +200ms대 스파이크). 호스트가 `audio-tempo` 보내도 게스트는 **네트워크 지연 동안 옛 speed 유지** → 그만큼 어긋남. P1-a anchor 리셋이 재정렬을 시도하나 전환 순간 위치 점프 자체는 수 초 잔존. (1→2 가속은 게스트가 잠깐 느린 거라 따라잡기 쉬워 덜 두드러짐 — 비대칭.)

**설계 방향 (코드 착수 전 합의 필요)**:
- 호스트가 즉시 적용 대신 **"wall T 시점에 speed S 적용"을 broadcast** → 양쪽이 같은 wall 시각에 동시 전환(네트워크 지연 무관). 기존 `schedule-play` 패턴 응용.
- 적용 방식: (a) Dart 타이머로 게스트가 T에 `setPlaybackSpeedX1000` 호출(native 무변경) vs (b) native "지정 시각 speed 변경" 지원.
- **위험**: schedule-play race 이력(v0.0.47 `_scheduleInProgress` — 동시 호출 시 큰 drift). speed 스케줄도 동일 race 가능 → 신중.

**결정 사항 (TODO, 사용자 합의)**:
- [ ] 스케줄 적용 방식 (a) Dart 타이머 vs (b) native 지원
- [ ] 스케줄 버퍼(T − now) 크기 (네트워크 지연 + 여유)
- [ ] race 방지 (schedule-play와 동일 가드 적용 여부)
- [ ] 전환 직후 강제 re-seek 필요 여부
