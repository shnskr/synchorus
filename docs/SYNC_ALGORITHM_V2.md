...
1. **사용자 합의** — H-2 항목 후 진행
2. **PoC 디렉토리 생성** `poc/transpose_engine/` — Flutter 격리 프로젝트
3. **SoundTouch NDK 통합** (이미 v0.0.91에서 검증된 빌드 패턴 재사용)
4. **Worker thread + lock-free ring 구현** — std::atomic + power-of-2 ring (single producer/single consumer)
5. **Algorithm latency 측정 인프라** — input frames put vs output frames received delta
6. **청감 검증 자동화** — 비프 음원 + ±12 sweep + glitch 카운트
7. **30분 stress + 측정 보고서**
8. **본 앱 통합 디자인** — outputLatencyMs 보정 식 + P2P broadcast
