...
### 2026-04-28 (46) — v0.0.51~v0.0.54 알고리즘 v2 그룹 1 구현 + 측정 검증 PASS

**배경**: (45) v0.0.49~v0.0.50 측정 인프라 + idle/burst baseline 측정으로 거짓말 패턴 + 누적 발산 + 메시지 race 메커니즘 확정. `docs/SYNC_ALGORITHM_V2.md` 디자인 문서 작성·합의 후 그룹 1 (D-1 보류, B 옵션) 단일 commit 구현.

**v0.0.51 알고리즘 v2 그룹 1 (단일 commit, 380 insertions)**:
- **A-2 결합 액션**: drift_ms + vf_diff_ms 4단계 분기 (정상/작은 보정/강제 reseek/emergency)
- **B-1 outputLatency EMA** (α=0.2): 매 poll raw 측정 → EMA 갱신, anchor reset 시 EMA 보존
- **F-2 강화 게스트 메시지 큐 + await 직렬화**: `_pendingObs/_pendingHostStateSync/_pendingGuestSeekTargetMs` + `_processQueue` 단일 thread + 우선순위 (host-state-sync > seek-notify > audio-obs). `unawaited(_startGuestPlayback)` race 제거 — guest_stop 32회 누락 root cause.
- **G-2 host-state-sync 보조 안전망**: 호스트 syncPlay/Pause 시 vf + playing + sampleRate broadcast. 게스트 강제 정렬 (vfDiff >30ms 시 seekToFrame).
- **호스트 cooldown debouncing**: syncPlay/Pause 200ms / syncSeek 100ms — 의미 없는 짧은 트랜지션 압축.
- **v0.0.56 anchor 중복 호출 버그 cherry-pick**: `_tryEstablishAnchor`의 `seekToFrame` 1번만 호출 (이전 2번).

**v0.0.51 측정 결과 — 회귀 발견 (idle 4분, 459 drift 샘플)**:

| | v0.0.49 baseline | v0.0.51 회귀 |
|---|---|---|
| drift_ms <5ms | 98.7% | 17.9% |
| drift_ms abs mean | 3.01ms | 31.54ms |
| drift_ms signed | -1.98ms | +29.48ms |
| vfDiff abs mean | 22.33ms | 34.90ms |
| vfDiff max | 60ms | 163.90ms |
| corrective_seek_small | 0 | **69회 (3.5초/번)** |

사용자 청감 "3:30~4:00 미묘 어긋남" 직접 인지. 회귀 메커니즘 분석 — corrective_seek가 oscillation + 누적 발산 일으킴:
- vfdiff_normal 30ms 임계 너무 빡셈 (idle baseline -21ms 베이크인이 자주 위반)
- 보정 후 `_seekCorrectionAccum += correctionFrames`만, anchorGF 갱신 안 함 → drift_ms가 보정량만큼 점프 (over-correction)
- 보정 진동 + 시간 따라 vfDiff 누적 (28ms → 48 → 116 → 144 → 163ms)

**v0.0.52 회귀 fix (4가지)**:
- **(1) 임계 완화**: vfdiff_normal 30→**80ms**, gain 0.5→**0.2**
- **(2) anchor 동기 reset**: 보정 후 `_anchorGuestFrame += correctionFrames` + `_driftSamples.clear()` → drift 점프 차단
- **(3) EMA 직접 보정**: 매 poll `_anchoredOutLatDeltaMs`를 EMA로 점진 수렴 (변화 50ms 이내 outlier 차단)
- **(4) medianWindow 5→10** (~2초 평균 노이즈 완화)

**v0.0.52 측정 결과 — 회귀 완전 해소 (idle 5분, 644 drift 샘플)**:

| | v0.0.49 baseline | v0.0.51 회귀 | **v0.0.52 fix** |
|---|---|---|---|
| drift_ms abs mean | 3.01ms | 31.54ms | **1.86ms** (baseline 향상) |
| drift_ms <5ms | 98.7% | 17.9% | **97.7%** |
| vfDiff abs mean | 22.33ms | 34.90ms | **20.71ms** (baseline 동등) |
| vfDiff max | 60ms | 163.90ms | **49.47ms** |
| vfDiff >50ms | 0.4% | 25.9% | **0%** |
| corrective_seek_small | 0 | 69 | **0** |

EMA 시간 변화 (vfDiff signed_mean per 30s): 0~30s +2.32ms (anchor 정확) → 30s 이후 -19~-22ms 안정 (baseline 수렴). EMA 직접 보정 작동 확인 — baseline 잔재까지 점진 수렴 + 임계 80ms 안에 머물러 보정 안 발동.

**v0.0.53 자동 정지 race fix**:
- v0.0.52 측정 후 사용자 보고 "5분 곡 끝났는데 재생시간 계속 올라감"
- Root cause: `_startTimestampWatch`의 자동 정지 로직이 매 100ms poll마다 `syncPause()` 호출 → debounce timer cancel + 200ms 갱신 → flush 영원히 안 됨
- **v0.0.53 fix**: `syncPlay/syncPause` idempotent guard. 같은 의도가 이미 pending이거나 현재 상태와 같으면 timer 갱신 안 함.

**v0.0.54 끝 도달 후 play race fix**:
- v0.0.53 측정 후 사용자 보고 "끝 도달 후 재생 누르면 마지막 소리 → 처음으로 → 정지"
- Root cause: `_flushHostState`에서 vf >= totalFrames 분기가 `await syncSeek(Duration.zero)` (debounce 큐) 호출 → seek 실제 적용 전에 `_engine.start()` 발동 → 마지막 위치에서 잠깐 재생 → 100ms 후 seek 처리 → 그 사이 stale vf 자동 정지 트리거
- **v0.0.54 fix**:
  - `_flushHostState` vf 끝 도달 분기에서 `_engine.seekToFrame(0)` 직접 await + broadcast (debounce 우회)
  - 자동 정지 로직에 `_pendingHostPlaying == null` 가드 추가 (debounce 진행 중 중복 트리거 차단)

**v0.0.54 측정 결과 — burst 검증 PASS (사용자 빠른 연타 2.7분, 1398 rows)**:

| | v0.0.50 burst | **v0.0.54 burst** | 효과 |
|---|---|---|---|
| host_play / pause | 175 / 175 | **81 / 80** | debouncing 절반 |
| guest_stop 누락 | 32회 (18%) | **5회 (6%)** | 6배 향상 |
| **vfDiff max** | **45269ms (45초)** | **702ms (0.7초)** | **64배 향상** |
| vfDiff abs mean | 3194ms | **28.52ms** | 100배 이상 |
| vfDiff >=1초 | 7.0% | **0%** | 누적 발산 완전 해소 |
| vfDiff 100-500ms | 0.4% | 0% | |
| corrective_seek_small | (측정 안 됨) | **0회** | 진동 없음 |
| anchor_reset_large_drift | (측정 안 됨) | 1회 | 700ms 한 번 발동 |
| drift_ms abs mean | 2.56ms | 1.87ms | 향상 |

**사용자 청감**:
- idle 5분: 어긋남 인지 0 ((45)의 v0.0.51 회귀 "3:30~4:00 미묘 어긋남" 사라짐)
- burst 2.7분: "seek bar 여러 번 옮길 때 2번 정도 잠깐 틀어짐, **1~2초 안에 회복**" — vfDiff 702ms 1번과 정확 일치, anchor_reset_large_drift 1회로 강제 reseek 발동
- 자동 정지: 곡 끝 도달 시 정지 OK + 재생 누르면 0:00부터 정상 재생 (마지막 소리 인지 안 됨)

**v0.0.54 그룹 1 효과 종합**:
- ✓ **거짓말 패턴 (idle 30~50ms 베이크인)** — A-2 + B-1 EMA로 baseline 수준 안정
- ✓ **누적 발산 (burst 45초)** — F-2 큐 + await 직렬화 + A-2 emergency 임계로 700ms 1회만, 1~2초 회복
- ✓ **메시지 race (guest_stop 32회 누락)** — 호스트 cooldown + F-2 큐로 5회로 감소
- ✓ **자동 정지 + 처음 재생** — v0.0.53/54 fix로 정상 동작
- ✓ **청감 검증**: idle 미인지, burst 1~2초 회복

**D-1 보류 결정 유지**:
- 그룹 1만으로 사고 시나리오 모두 해결. 청감 미인지 한계 영역에서 안정 운영
- D-1 (anchor 분리)은 추가 효과 작을 것으로 평가 — 향후 30분+ idle / iOS host / BT 환경 측정에서 필요성 재검토

**검증 안 된 시나리오 (다음 세션 후보)**:
1. **30분+ 장시간 idle** — rate drift 누적 검증 (현재 5분만, drift_ms <5ms 97.7%이라 누적 가능성 작지만 미검증)
2. **iOS host 환경** — outputLatency 보고 정밀도 차이. Mac 환경 필요.
3. **BT 환경** — BT 비대칭 + EMA 학습 상호작용 (40~200ms BT outputLatency 변동)
4. **다중 게스트 (1:N)** — 1대1 검증만, 1대N 미확인

**알려진 잠재 인지 변화 (의도된 동작)**:
- **debounce 200ms**: 사용자가 100ms 안에 두 번 누른 정지/재생은 한 번만 적용
- **F-2 큐 coalescing**: 빠른 정지/재생 100ms 안 토글 시 중간 의도 손실 — race 차단의 trade-off
- 측정에서 청감 인지 안 됨 → OK

**측정 csv 보존**: `measurements/v0.0.51_idle_2026-04-28.csv`, `v0.0.52_idle_2026-04-28.csv`, `v0.0.54_autostop_2026-04-28.csv`, `v0.0.54_burst_2026-04-28.csv`. v0.0.49 baseline + v0.0.50 burst와 직접 비교 가능.

**변경 범위**: `lib/services/native_audio_sync_service.dart` (메시지 큐 + debounce + A-2 액션 + B-1 EMA + 새 상수, 약 380 insertions). sync 메커니즘만 영향. 다운로드/P2P/discovery/UI/라이프사이클 변경 0.

**디자인 문서 갱신**: `docs/SYNC_ALGORITHM_V2.md` 빈틈 #2/#3/#13/#14/#15 명세 + B 옵션 합의 + EMA α=0.2 + medianWindow 10 + 자동 정지 race 추가 fix.

---

### 2026-04-28 (47) — Tab A7 Lite 호스트 + S22 게스트 환경 진단 (수정 X, 기록만)

**배경**: (46) 그룹 1 검증 완료 후 사용자가 추가 테스트로 역할 반전 환경(Tab A7 Lite 호스트 + S22 게스트)에서 측정. 1:40쯤부터 갑자기 어긋남 청감 인지. csv + logcat 진단 — 출시 관점에서 어떤 기기든 호스트가 될 수 있어 fix 필요성 확인.

**측정 결과 (3.5분, 569 rows)**:

| | S22 host (정상, v0.0.54) | **Tab A7 Lite host (역할 반전)** |
|---|---|---|
| drift_ms abs mean | 1.86ms | **77.89ms** (40배) |
| drift_ms max | 8.37ms | **178ms** |
| drift_ms signed | -1.20ms | **-77.33ms** |
| vfDiff abs mean | 20.71ms | **66.99ms** |
| vfDiff max | 49ms | **187ms** |
| corrective_seek_small | 0 | **27회 진동** |

**진단 핵심 — 호스트 framePos vs vf 비대칭 누적 (logcat 직접 캡처)**:

```
35s:   host fpVfDiff = -23.0ms   (정상, HAL latency)
57s:   host fpVfDiff = -46716ms (-46초)   ← 폭증 시작
77s:   host fpVfDiff = -95018ms (-95초)   ← 더 폭증
135s:  host fpVfDiff = +27218ms (+27초)   ← 부호 반전
```

**root cause 가설**: Tab A7 Lite 저가형 oboe HAL의 pause/resume 사이클 시 **stream framePos는 누적**되는데 **콘텐츠 vf는 정지/seek 따라 진행** → 시간 따라 baseline 비대칭 누적. 사용자 활동 (정지/재생/seek) 많을수록 더 큼. HISTORY (43) 알려진 이슈 ("Tab A7 Lite oboe pause/resume xrun + ErrorInvalidState") 직접 연관.

**메커니즘 4단계**:
1. 호스트 측 framePos가 콘텐츠 vf와 분/초 단위 어긋남 누적
2. anchor establish 시 게스트가 호스트 framePos baseline으로 베이크인
3. 게스트 자기 framePos baseline (HAL 카운터)와 비대칭 비교 → drift_ms가 일정 offset (-127ms)으로 박힘
4. corrective_seek 27회 진동 무효 (`_anchorGuestFrame += correctionFrames` 동기 갱신만 게스트 측, 호스트 baseline 비대칭 못 잡음)

**Rate는 정확** (host_obs_wall 기준 호스트 vf 진행 44100/s ± 1%). 즉 rate drift 아니라 **절대 baseline 비대칭** 문제.

**HOST-STATE-SYNC forced seek는 부분 작동** — vfDiff 큰 차이 감지 시 강제 정렬. 그래서 vfDiff는 100~190ms (45초 폭발 안 됨)지만 drift는 -127ms 안정 누적.

**출시 관점 결정**: 사용자 지적 "어떤 기기에서 사용할지 모름" 정합. 정상 사용 시나리오에서도 저가형 호스트 가능성 → fix 필요. 다음 세션 우선순위 1.

**다음 세션 fix 후보**:

1. **D-1 anchor 분리** (우선순위 ↑↑, (47)이 직접 근거)
   - rate anchor (framePos 기반): 진행률 측정 — 호스트 framePos rate은 정확하니 rate drift만 측정
   - position baseline (vf 기반): 절대 위치 정렬 — host-state-sync로 vf 강제 정렬, framePos baseline 비대칭 영향 안 받음
   - 디자인 빈틈 #1 보강 필수: 두 anchor 상호작용, 첫 establish 시점, reset 매트릭스 명세

2. **호스트 측 framePos 정규화** (보조 fix)
   - `oboe_engine.cpp` `getTimestamp` 분기에서 framePos를 콘텐츠 frame으로 normalize
   - oboe pause/resume 후 framePos가 stream rendering이 아닌 콘텐츠 vf와 일치하도록 보정
   - 또는 `obs.framePos`를 송신할 때 vf 기반으로 정규화 (Dart 측)
   - D-1만으로 부족하면 추가

3. **Tab A7 Lite oboe pause/resume 회피** (선택)
   - v0.0.46 oboe stop을 close → pause/resume 모델로 변경 — 저가형 HAL 한계
   - 이전 close + reset 모델로 분기 (저가형 기기 감지 시) 또는 전체 회귀

**측정 데이터 보존**: `measurements/v0.0.54_reverse_2026-04-28.csv`, `measurements/v0.0.54_reverse_guest_logcat.txt` (S22 게스트 측 [ANCHOR]/[CORRECTIVE-SEEK]/[HOST-STATE-SYNC] 로그).

**변경 범위**: 코드 변경 0. 진단 + 기록만.
