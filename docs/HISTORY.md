...
**레이턴시 보정**
- [ ] S22 buf=4ms vs iPhone buf=21ms 비대칭 (17ms) — 자동 보정 알고리즘 정밀도 개선 우선. (수동 슬라이더 도입 여부는 사용자 명시 요청 시 결정)
...
- [ ] **v0.0.27 errno=111 빠른 포기 실측 검증** — T4b(재생 전 호스트 강제 종료) + W(게스트 WiFi off/on)는 Android 에뮬 adb forward 환경에서 TCP `Socket.connect`가 항상 가짜 성공 반환해 경로 진입 자체 불가 (2026-04-24 (22) 참조). 실기기 LAN 조합(S22 + iPhone 또는 Android 실기기 2대) 필요.
