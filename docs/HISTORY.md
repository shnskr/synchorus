...
### 2026-06-16 (156) — 가이드 다시보기 highlight 좌측 치우침 fix (v0.0.128)

**증상**: 설정 → "사용법 가이드 다시 보기" 시 첫 타겟(파일 카드) coach mark highlight가 카드 폭보다 좁게 좌측으로 잡힘. **단, 첫 실행 자동 가이드는 정상**.

**진단 (픽셀 측정으로 확정)**: 첫 실행 카드 focus 우측=1074px(full width 정상) vs 다시보기=812px(80%, 좌측 쏠림). 시크바 등 다른 타겟은 둘 다 정상(1067) → 전역 아니고 **첫 타겟 + 다시보기 경로** 한정. **헛다리 배제**: 배율(345/450)·배너·작은화면 스크롤 래핑(IntrinsicHeight 제거해도 동일)·SizedBox(infinity) 래핑·postFrame 1프레임 — 모두 무효(812 유지). 한때 "패키지 한계"로 오판했으나 **사용자 지적("첫 실행은 되는데 다시보기만 안 됨, 설정창이 옆으로 사라지는 전환 중 호출하는 것 아니냐")이 root cause를 정확히 짚음**.
...
진단 로그(세션 한정): `/tmp/synchorus_repro.log`(stuck), `/tmp/synchorus_clean.log`(정상). **출시 차단급** → PLAN HIGH 등록. **v0.0.133 = T1만(안전망 — 스와이프 복구).**

**T2 watchdog 구현 + 검증 완료 (v0.0.134, 2026-06-20)**
- 구성: native `reopenStream()`(출력 스트림만 close→reopen, vf·ring·파일 보존, ST out-ring은 mPitchDirty/Tempo로 워커 안전 clear, mDebugForceStuck 해제) + Dart watchdog(100ms, `_playing`+EOS+seek쿨다운+백오프 게이트) + reopen 후 `_resetDriftState()` 재정렬 + 지수 백오프(1→16s, 정상 30s 지속 시 카운트 리셋) + on/off 컴파일 상수 `_kWatchdogEnabled`. **Android 게이트**(iOS는 `rebuildEngineAndResume` 보유, fast-follow).
- **검증 인프라**: debug 전용 강제-stuck 훅(`mDebugForceStuck` → onAudioReady가 vf++ 전 무음 반환) + player_screen `if(kDebugMode)` 🐛 버튼(release 자동 제외). "재현 불가 버그"를 버튼으로 재현.
- ⚠️ **핵심 디버그 교훈**: watchdog 1차 구현이 `_engine.latest.virtualFrame`(=getLatestTimestamp가 framePos 시점으로 **외삽 보정**한 값, oboe_engine.cpp:556-559)을 읽어 **±수천 frame 진동 → 동결 미감지**(실측: 284844→285340→286613→284525). **raw `mVirtualFrame`(`getVirtualFrame()`, 보정 없음 — 오디오 콜백만 전진)** 으로 교체해 해결. raw vf = "콜백 살아있나"의 정확한 신호.
- **실측 (SM S947N debug)**: 솔로 🐛 → `[WATCHDOG] vf stuck 300ms → reopen` → `reopenStream: OK (vf 보존)` → `calcLatency recovered ~4~8ms`, 매번 즉시 복구. **P2P(S947N 호스트 + S22 게스트)**: 호스트 stuck 3회 + 게스트 stuck 1회 전부 복구, 청감상 재정렬 정상(사용자 관찰). **검토 🔴 "host framePos 리셋→게스트 drift 폭발"은 기존 drift 임계 자가복구로 해소 → obs-epoch 불요**(미구현 유지). ⚠️ 게스트 re-anchor 이벤트는 측정 CSV로 빠져 logcat 미확인 — 청감 관찰로만 확정.
- 미구현(후순위): T3 Oboe `onErrorAfterClose`(T2가 disconnect도 포섭하므로 폴리시) / iOS reopenStream / Exclusive→Shared 폴백 / obs-epoch(현재 불요).

### 2026-06-20 (163) — 스플래시 표시 시간 평탄화: 최소 1초 보장 (preserve/remove, v0.0.135)

사용자 관찰: 앱 시작 시 스플래시가 "어떤 땐 길게, 어떤 땐 거의 0초로 휙" 지나가 일관성이 없음.

**원인 (코드 확정)**
- 스플래시는 `flutter_native_splash` 네이티브 전담(Android LaunchTheme / iOS LaunchScreen)이고 `preserve`/`remove` 수동 제어가 없었음 → **첫 Flutter 프레임 렌더 순간 자동 제거**. 최소 표시 시간이 0이라 "첫 프레임까지 걸린 가변 시간"만큼만 보임.
- 그 가변 시간 = ① 엔진·Dart VM 로드(콜드/웜 편차 큼) + ② `main()`의 `runApp` 전 await 체인. main 점검: AdMob은 `unawaited`(`main.dart:44`)라 시작 안 막음 ✅. `AudioService.init`(`:70`, 가장 의심)·AudioSession·첫 실행 알림아트 복사가 await. **1초+ 명백한 병목은 정적 분석상 안 보임**(AudioService.init 실측 미수행 = 가설).

**결정 (B안 — Dart만)**
- A안: 네이티브 채널(Android Kotlin + iOS Swift)로 **프로세스 시작 시각**을 받아 1초를 맞춰 엔진로드 변동까지 흡수. B안: Dart만, **main 진입 기준** 1초.
- 정식 출시 마무리 단계라 양 플랫폼 네이티브 면적 추가 리스크 회피 + 현 불만 핵심("0초 깜빡임")은 B로 해소 → **B안 채택**. 잔여 변동이 거슬리면 후속 A안 승급.

**구현 (v0.0.135)**
- `flutter_native_splash`를 dev_dependencies → dependencies 이동(런타임 preserve/remove import 위해). pubspec.lock 1건 변경.
- `main()`(`main.dart:28~`): `ensureInitialized()` 반환 binding으로 `FlutterNativeSplash.preserve()` → 첫 프레임 이후에도 스플래시 유지. `Stopwatch`로 main 진입부터 경과 측정. `runApp` 직후 경과 < `_kMinSplashMs`(1000)면 남은 만큼 `Future.delayed` 후 `remove()`, 이미 넘었으면 즉시.

**한계 (정직)**
- main 진입 기준이라 **엔진 로드 구간(프로세스 시작~Dart 진입)은 미포함** → 체감 ≈ 엔진로드 + 1초. 콜드/웜 엔진로드 변동은 일부 잔존. 완전 평탄화는 A안(네이티브) 필요. 변동 크기 미실측 → 승급은 데이터 확인 후.
- `flutter analyze lib/main.dart` 통과(No issues). **실기기 체감 검증은 다음 빌드 시(현재 미수행).**

---

### 2026-06-23 (164) — A-B 구간 반복: 미니플레이어 시크가 [A,B] 무시 fix (v0.0.136)

사용자 관찰: A-B 구간을 설정해두면 **앱 안에서는** A 앞/B 뒤로 이동이 막히는데, **미니플레이어(알림/잠금화면) 시크바**에선 구간 밖으로 자유롭게 이동됨.

**원인 (코드 확정)**
- A-B 상태(`_abPointA/_abPointB`)와 clamp 로직이 전부 `player_screen.dart`(UI 레벨)에만 존재. 인앱 seek 4경로(시크바 `:630`, ±5초 `:339`, 슬롯 `:297`, B자동점프 `:229`)는 모두 `syncSeek` **호출 전에** `[A,B]`로 clamp.
- 반면 미니플레이어 seek은 `audio_handler.dart`의 `seek()`(SeekHandler override) → `_syncService.syncSeek(position)`을 **clamp 없이** 직접 호출. `syncSeek`은 파일 범위 `[0,totalFrames]`로만 clamp(`native_audio_sync_service.dart:637`)라 A-B를 모름 → 구간 밖 seek 통과.
- audio_handler가 외부 seek을 받는 **유일한** 경로라, 알림뿐 아니라 블루투스 AVRCP·안드로이드 오토·카플레이 시크도 동일하게 우회.

**설계 검토 — 2안 비교 후 단순안 채택**
- B안(창 리맵): 알림에 보고하는 duration을 `(B−A)`, position을 `(실제−A)`로 오프셋 → 알림 바 전체가 A-B 창이 되어 드래그가 물리적으로 못 벗어남. **대가: 알림 시간 표시가 곡 전체가 아닌 창 기준(0~창길이)으로 바뀜** → 사용자가 "곡이 30초밖에 안 되네?" 혼란. 사용자 기각.
- A안(스냅백, 채택): 알림 바는 곡 전체 그대로(시간 표시 정상). 드래그는 자유지만 **놓는 순간 `[A,B]`로 clamp + 보정 위치 emit** → 썸이 경계로 스냅백. "끌어도 도로 들어온다".
- audio_service `^0.18.18`에는 시크바 **부분 범위(min/max seek) 지정 API가 없음** — 바 범위는 `MediaItem.duration`(0~duration) 하나로만 결정(`audio_service.dart:185`, PlaybackState 위치 필드는 `updatePosition`/`bufferedPosition`뿐). 그래서 "풀 곡 바 유지 + 드래그 물리 차단"은 불가, A안(결과 보정)/B안(창 리맵) 둘 중 택일.

**스냅백 가시성 함정 (코드 확정 → A안 핵심)**
- `audio_handler`의 positionStream 리스너는 `_lastPosition`만 갱신하고 `_emitPlaybackState()`를 **재호출하지 않음**(`audio_handler.dart:44~46`). `_emitPlaybackState`는 play/pause/duration/loading 때만 + 주기 타이머 없음.
- 서비스 주기 poll(`native_audio_sync_service.dart:1404`)도 seek 후 500ms `_seekOverridePosition`으로 억제됐다 풀려도 positionController에만 흘러갈 뿐 OS로 재push 안 됨.
- → **`syncSeek`에서만 clamp하면 OS 시크바 썸은 드래그 자리에 남고 거기서 앞으로 흐름**(재생만 막힘). 따라서 clamp+emit을 **audio_handler.seek 안에서 한 덩어리로** 처리해야 스냅백이 보임.

**구현 (v0.0.136, 2파일 — `syncSeek`/서비스 무변경)**
- `audio_handler.dart`: `_loopStart/_loopEnd` 필드 + `setAbLoop(start,end)` setter + `_clampToLoop()`. `seek()`에서 clamp → `_lastPosition` 보정 → `_emitPlaybackState()` → `syncSeek(clamped)`. broadcast `targetMs`도 clamp된 값이라 게스트 일관. `detachSyncService()`에서 경계 reset.
- `player_screen.dart`: `_syncAbToHandler()` 헬퍼 추가 — active면 `setAbLoop(_effectiveA,_effectiveB)`, 비활성이면 `setAbLoop(null,null)`. A-B 변경 4곳(`_setAbPoint`/`_resetAb`/`_clearAbPoint`/파일변경 시 durationSub reset)에서 호출.

**검증/한계 (정직)**
- `flutter analyze` 2파일 통과(No issues). **실기기 동작 검증 미수행.**
- 가설(미실측): OS가 드래그 **중** 연속 seek을 쏘면 경계 반복 clamp로 미세 stutter 가능(대개 release만이라 실제론 드묾). iOS 잠금화면 스크러버가 `updatePosition` push에 즉각 스냅백하는지 sticky 여부 — 실기기 확인 필요.
- **비공개 테스트 빌드**: `flutter build appbundle --release --no-tree-shake-icons` → `app-release.aab` 75.8MB 생성. 빌드 번호 0.0.136 **+5→+6**(versionName 유지, 재업로드용 versionCode 중복 회피 — (160) 패턴). minify OFF·release 서명 적용 확인.

---

### 2026-06-30 (165) — 정식 출시: 통신판매업 신고 완료 + 비공개(closed) 테스트 게시 → 14일 카운트 진입 (코드 변경 없음)

출시 마무리 트랙 진행 기록. 코드 변경 없음(상태/일정 기록).

- ✅ **통신판매업 신고 완료** — 정부24 접수(2026-06-19)·등록면허세 납부 후 신고증 발급. 신고번호로 Play Console **계정 세부정보 3칸**(사업자등록번호 + 전자상거래 라이선스 번호=통신판매 신고번호 + 라이선스 대행사=신고기관) 입력 → **한국 전자상거래법 게시 차단 해제** (이전 (162)/(PLAN)에서 closed "변경사항 제출"까지 막던 빨간 배너 해소).
- ✅ **비공개 테스트(closed) 게시 + 테스터 12명 모집 완료**. 게시 차단이 풀려 closed 트랙 릴리스 제출·게시 가능해짐. 프로덕션 액세스 요건(신규 개인계정 2023.11.13 이후)인 테스터 12명 opt-in 충족.
- 🔄 **현 단계 = 비공개 테스트 7일째(2026-06-30 기준, D+7)**. 연속 14일 카운트 진행 중 → **~2026-07-07 충족 예정**. 이메일 등록 ≠ 카운트 시작이지만 12명 실제 opt-in(링크 수락+설치) 완료 상태.
- ⏳ **이후**: 14일 충족 → 프로덕션 액세스 신청 → 버전 0.0.132→1.0.0 프로덕션 승격. iOS 라운드(AdMob iOS 실 ID/UMP·스플래시/아이콘 실기기·알림 권한·release 회귀)는 별도.
- 상세 진행 표·남은 순서: [PLAN.md](PLAN.md) "출시 마무리 트랙".
