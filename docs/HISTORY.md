...
- [ ] 호스트 파일 빠른 교체 시 race condition — 게스트 다운로드 실패 / 이전 곡 재생
- [ ] 게스트 파일 다운로드 속도 체감상 느림 (HTTP shelf_static, 청크 크기/버퍼 확인)
- [ ] iOS 게스트에서 총 재생시간(duration) 0:00 표시 — iOS AudioEngine의 totalFrames/sampleRate 반환값 확인
- [ ] Android↔iOS 싱크 정확도 정밀 실측 필요

**레이턴시 보정**
- [ ] S22 buf=4ms vs iPhone buf=21ms 비대칭 (17ms) — 수동 offset 슬라이더로 대응 검토
- [ ] 엔진 레이턴시 보정값 ~10ms 오차 — 수동 보정 슬라이더 추가 예정

**안정성**
- [ ] 호스트 백그라운드 진입 시 파일 서버 끊김 → 게스트 seek 시 404
- [ ] 호스트 파일선택 창 열고 있는 동안 게스트 입퇴장 시 안정성
