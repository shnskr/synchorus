...
| §G-1 PCM ring buffer (v0.0.76) | Android 사전할당 PCM 전체 디코딩 → 60s sliding window ring buffer로 교체. behind 10s + ahead 50s 분배, modular index, decodeLoop 단일 producer + 외부 consumer/seek 분리. `TOO_LONG` 14분 한도 완전 제거 → 51분 곡 로드 검증, 메모리 ~11.5MB constant (곡 길이 무관, 이전 5분 곡 ~58MB → 14분 곡 OOM). 큰 seek = 윈도우 안 즉시(vf만) / 밖 head/tail reset + decodeLoop 점프 트리거. `fillGaps` 폐기(sliding window라 갭 개념 없음). 이전 2-range 추적(`[0, seqEnd)` + `[seekStart, seekEnd)`)도 단일 윈도우 비교로 단순화. iOS는 AVAudioFile 자체 스트리밍이라 변경 0. 기존 row "Android 파일 디코딩: NDK AMediaCodec 전체 메모리 디코딩"을 갱신. |
...
| Android 파일 디코딩: NDK AMediaCodec 전체 메모리 디코딩 | 스트리밍보다 단순, 150MB 제한으로 ~5분 곡 커버. iOS는 AVAudioPlayerNode가 자체 스트리밍. **→ v0.0.76 §G-1에서 ring buffer 60s sliding window로 갱신 (표 상단 §G-1 row 참조)**. |
